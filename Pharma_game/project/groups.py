...
class AllSprites(pygame.sprite.Group):
	...
	def draw(self, player):
		self.offset.x = round(-(player.rect.centerx - WINDOW_WIDTH / 2))
		self.offset.y = round(-(player.rect.centery - WINDOW_HEIGHT / 2))
		...
		for layer in (bg_sprites, main_sprites, fg_sprites):
			for sprite in layer:
				if isinstance(sprite, Entity):
					self.display_surface.blit(self.shadow_surf, sprite.rect.topleft + self.offset + vector(20,50))
